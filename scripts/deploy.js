...
async function main() {
    ...
    const Bigboybar = await ethers.getContractFactory("BigBoyBar", deployer);
    const bbcToken = await ethers.getContractAt("contracts/bbcreferral.sol:IBEP20", process.env.BBC_TOKEN, deployer);

    const referral = await ethers.getContractAt("BBCReferral", process.env.REFERRAL, deployer);
    ...
    const bbcBNBLp = await ethers.getContractAt("contracts/bbcreferral.sol:IBEP20", process.env.BBC_BNB, deployer);
